...
void get_parent_index_test()
{
  ...
  CU_ASSERT_EQUAL(get_parent_index(3), 1);
  CU_ASSERT_EQUAL(get_parent_index(4), 2);
...
void get_left_index_test()
{
  ...
  CU_ASSERT_EQUAL(get_left_index(3), 6);
  CU_ASSERT_EQUAL(get_left_index(4), 8);
}
...
void get_right_index_test()
{
  ...
  CU_ASSERT_EQUAL(get_right_index(3), 7);
  CU_ASSERT_EQUAL(get_right_index(4), 9);
}
...
void get_heap_size_test()
{
  ...
  CU_ASSERT_EQUAL(get_heap_size(aux), 26);
}
...
void enqueue_huffman_heap_test()
{
  ...
  item_of_node = (unsigned char*)malloc(sizeof(unsigned char));
  *item_of_node = '*';
  node = create_huffman_tree_node(item_of_node, 1, NULL, NULL);
  enqueue_huffman_heap(aux, node);
  CU_ASSERT_EQUAL(*(unsigned char*)(((huffman_tree*)aux->data[1])->item), '*');
...
int main() 
{
  ...
  if (NULL == CU_add_test(pSuite, "get_heap_size_test", get_heap_size_test))
  {
    CU_cleanup_registry();
    return CU_get_error();
  }
